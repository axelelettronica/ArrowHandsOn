...
int sme_cdc_mgr_init (void)
{
	...
	xTaskCreate(cdc_task_rx, (const char *) "CDC", configMINIMAL_STACK_SIZE,
		        NULL, CDC_TASK_PRIORITY, NULL);
	return 0;
}
	...
	for (;;) {
		...
		while (xQueueReceive(cdc_msg_in_queue, current_char_ptr, 0)) {
			...
			case '\r':
                ...
                //print_dbg("parsing command %s\r\n", (int8_t *)sme_cli_msg.token[0]);
			...
			case '\b':  // backspace
		        ...
		        *current_char_ptr = '\0';
			    ...
			    if (current_column) {
				    --current_column;
			    } else  if (sme_cli_msg.token_idx) {
                    // previous token
                    sme_cli_msg.token_idx--;
                    current_token_ptr = sme_cli_msg.token[sme_cli_msg.token_idx];
                    current_column = strlen(current_token_ptr);
                }
