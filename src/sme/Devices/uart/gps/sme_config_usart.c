...
#define GPS_SERCOM_PINMUX_PAD0  PINMUX_PB08D_SERCOM4_PAD0
#define GPS_SERCOM_PINMUX_PAD1  PINMUX_PB09D_SERCOM4_PAD1
...
#define SME_GPS_RX_BUFFER_LENGTH   5 //MAX_SL868V2_RX_BUFFER_LENGTH

/* interrupt USART variables */
static struct usart_module usart_gps;

volatile uint8_t gps_rx_buffer[SME_GPS_RX_BUFFER_LENGTH];
/* interrupt USART variables */
...
{
	memset((char *)gps_rx_buffer,0,SME_GPS_RX_BUFFER_LENGTH);
...
{ 
	status_code_genare_t ret =  usart_read_buffer_job(&usart_gps, (uint8_t *)gps_rx_buffer, 
                                                      SME_GPS_RX_BUFFER_LENGTH);
	if (ret == STATUS_OK) {
		memcpy((char *)msg, (char *)gps_rx_buffer, SME_GPS_RX_BUFFER_LENGTH);
        return SME_OK;
	} else {
        return SME_EIO;
    }
